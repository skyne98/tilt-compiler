...
fn test_vm_jit_compatibility(
    ...
    let vm_result = vm.call_function(function_name, args.clone())
        .map_err(|e| format!("VM execution failed: {:?}", e))?;

    // Test with JIT
    let jit_host_abi = Box::new(NullHostABI::new());
    let mut jit = JIT::new_with_abi(jit_host_abi)
    ...
    let func_ptr = jit.get_func_ptr(function_name)
    ...
    let jit_result = unsafe {
        match args.len() {
            0 => {
                ...
                let func = program.functions.iter()
            ...
            1 => {
                let arg0 = match &args[0] {
                    ...
                    RuntimeValue::Usize(val) => *val as i32,
                    RuntimeValue::Void => 0,
                };
            ...
            3 => {
                ...
                if func.return_type == Type::Void {
                    ...
                    let func_fn = std::mem::transmute::<*const u8, fn(i32, i32, i32) -> i32>(func_ptr);
...
fn create_arithmetic_program() -> tilt_ir::Program {
    ...
    let func_idx = builder.create_function("add_mul", vec![Type::I32, Type::I32, Type::I32], Type::I32);

    {
        ...
        let a = func_builder.add_block_param(entry, Type::I32);
        let b = func_builder.add_block_param(entry, Type::I32);
        let c = func_builder.add_block_param(entry, Type::I32);

        let sum = func_builder.ins().binary_op(tilt_ir::BinaryOperator::Add, Type::I32, a, b);
        let result = func_builder.ins().binary_op(tilt_ir::BinaryOperator::Mul, Type::I32, sum, c);
...
fn create_conditional_program() -> tilt_ir::Program {
...

    {
        ...
        let cmp = func_builder.ins().binary_op(tilt_ir::BinaryOperator::Lt, Type::I32, b, a);
...
fn create_recursive_program() -> tilt_ir::Program {
...

    {
        ...
        func_builder.switch_to_block(entry);
        let n = func_builder.add_block_param(entry, Type::I32);

        // Check if n <= 1
        let one = func_builder.ins().const_i32(1);
        let cmp = func_builder.ins().binary_op(tilt_ir::BinaryOperator::Lt, Type::I32, one, n);
        ...
        let n_minus_1 = func_builder.ins().binary_op(tilt_ir::BinaryOperator::Sub, Type::I32, n, one);
        let factorial_result = func_builder.ins().call("factorial", vec![n_minus_1], Type::I32);
        let result = func_builder.ins().binary_op(tilt_ir::BinaryOperator::Mul, Type::I32, n, factorial_result);
        func_builder.ins().ret(Some(result));
    }
...
fn create_division_program() -> tilt_ir::Program {
...

    {
        ...
        let is_zero = func_builder.ins().binary_op(tilt_ir::BinaryOperator::Eq, Type::I32, b, zero);
        ...
        let result = func_builder.ins().binary_op(tilt_ir::BinaryOperator::Div, Type::I32, a, b);
        func_builder.ins().ret(Some(result));
    }
...
mod tests {
    ...
    fn test_arithmetic_compatibility() {
        ...
        let args = vec![RuntimeValue::I32(5), RuntimeValue::I32(3), RuntimeValue::I32(2)];
    ...
    fn test_edge_cases() {
        ...
        // Test with large numbers (but avoid overflow)
        let args1 = vec![RuntimeValue::I32(1000), RuntimeValue::I32(0), RuntimeValue::I32(1)];
        test_vm_jit_compatibility(program.clone(), "add_mul", args1)
            .expect("Edge case test (large numbers) should pass");

        // Test with negative numbers
        let args2 = vec![RuntimeValue::I32(-10), RuntimeValue::I32(5), RuntimeValue::I32(-2)];
    ...
    fn test_zero_operations() {
        ...
        // Test with all zeros
        let args = vec![RuntimeValue::I32(0), RuntimeValue::I32(0), RuntimeValue::I32(0)];
        test_vm_jit_compatibility(program, "add_mul", args)
